...
const { trackNodes, totalTrackLength, getPointAtDistance, findRearDistance } = useTrack()
...
function loop() {
    if (ctx && engine) {
        ...
        // デバッグ情報更新
        debugInfo.calculatedChordLength = Math.hypot(pFront.x - pRear.x, pFront.y - pRear.y)
    ...
    <div class="flex flex-col items-center bg-[#181818] text-gray-200 p-5 min-h-screen">
        ...
        <div
            ...
            <div class="flex items-center gap-4">
                <span>設定台車間距離 (L): <strong class="text-blue-400 font-bold">{{ params.bogiePitch.toFixed(1) }}
                        px</strong></span>
                <span class="text-neutral-600">|</span>
                <span>実際の弦長: <strong class="text-emerald-400 font-bold">{{ debugInfo.calculatedChordLength.toFixed(2) }}
                        px</strong></span>
            </div>
