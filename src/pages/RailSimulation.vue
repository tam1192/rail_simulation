...
onMounted(async () => {
    try {
        ...
        console.error(error);
        wasmError.value =
            error instanceof Error ? error.message : "WASM init failed";
    }
});
...
<template>
    ...
    >
        ...
        >
            <label
                class="flex items-center gap-2 cursor-pointer select-none text-sm text-content-main"
            >
                <input
                    type="checkbox"
                    v-model="params.useCorrection"
                    class="rounded accent-brand cursor-pointer"
                />
            ...
            <label class="flex items-center gap-2 text-sm text-content-main">
                ...
                />
                <span class="w-6 text-right font-mono text-content-muted">{{
                    params.speed
                }}</span>
            </label>
            ...
            <label class="flex items-center gap-2 text-sm text-content-main">
                ...
                <input
                    ...
                    class="accent-brand cursor-pointer"
                />
                <span class="w-12 text-right font-mono text-content-muted"
                    >{{ params.bogiePitch }}px</span
                >
            </label>
        </div>

        <p v-if="wasmError" class="text-rose-500 text-sm mb-2">
            {{ wasmError }}
        </p>
        <canvas
            ref="canvasRef"
        ...
        >
            ...
            <p
                v-if="!params.useCorrection"
                class="text-rose-600 dark:text-rose-400 text-xs mt-1"
            >
